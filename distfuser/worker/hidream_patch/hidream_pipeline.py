# Adapted from https://github.com/HiDream-ai/HiDream-I1/blob/main/hi_diffusers/pipelines/hidream_image/pipeline_hidream_image.py
...
from .xdit_context_parallel import HiDreamAttnProcessor_usp, usp_dit_forward
...
if HAS_HIDREAM_INSTALLED:
    from hi_diffusers.pipelines.hidream_image.pipeline_hidream_image import calculate_shift, retrieve_timesteps
...
# region TextEncoder
class HiDreamTextEncoderPipeline:
    def __init__(
        self,
        checkpoint_dir: str,
        ...
        self.tokenizer_4 = PreTrainedTokenizerFast.from_pretrained(llama_model_name, use_fast=False)
    ...
    def _get_llama3_prompt_embeds(
        ...
        outputs = self.text_encoder_4(
            text_input_ids.to(device), attention_mask=attention_mask.to(device), output_hidden_states=True, output_attentions=True
        )
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_4 = batch_size * [negative_prompt_4] if isinstance(negative_prompt_4, str) else negative_prompt_4
    ...
    def generate(
        ...
        return prompt_embeds, pooled_prompt_embeds, batch_size


# region DiT
class HiDreamDiTPipeline:
    def __init__(
        self,
        checkpoint_dir: str,
        model_type: str = "full",
        ...
        if use_usp:
            for block in self.transformer.double_stream_blocks:
                block.block.attn1.set_processor(HiDreamAttnProcessor_usp(use_sage_attn=use_sage_attn, sage_type=sage_type))

            for block in self.transformer.single_stream_blocks:
                block.block.attn1.set_processor(HiDreamAttnProcessor_usp(use_sage_attn=use_sage_attn, sage_type=sage_type))

            self.transformer.forward = types.MethodType(usp_dit_forward, self.transformer)

        if model_type == "full":
            self.scheduler = FlowUniPCMultistepScheduler(num_train_timesteps=1000, shift=shift, use_dynamic_shifting=False)
        elif model_type == "fast" or model_type == "dev":
            self.scheduler = FlashFlowMatchEulerDiscreteScheduler(
    ...
    def generate(
        ...
        prompt_embeds = [embed.to(self.device) for embed in prompt_embeds]
        ...
        # Chunk embed when using cfg parallel
        if self.do_classifier_free_guidance and self.use_cfg_parallel:
            cfg_rank = get_classifier_free_guidance_rank()
            for i, embed in enumerate(prompt_embeds):
                if embed.ndim == 3:
                    prompt_embeds[i] = embed.chunk(2, dim=0)[cfg_rank]
                else:
                    prompt_embeds[i] = embed.chunk(2, dim=1)[cfg_rank]
            pooled_prompt_embeds = pooled_prompt_embeds.chunk(2, dim=0)[cfg_rank]
        ...
        if isinstance(self.scheduler, FlowUniPCMultistepScheduler):
            self.scheduler.set_timesteps(num_inference_steps, device=self.device, shift=math.exp(mu))
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            if self.do_classifier_free_guidance and not self.use_cfg_parallel:
            ...
            if self.do_classifier_free_guidance:
                if self.use_cfg_parallel:
                    noise_pred = get_cfg_group().all_gather(noise_pred, dim=0)
